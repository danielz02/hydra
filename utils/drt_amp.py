...
from utils.utils_ensemble import get_stats, grad_l2, stability_loss, cosine_diversity
    ...
    for i, batch in enumerate(dataloader):
        ...
        for inputs, targets in mini_batches:
            ...
            for j in range(args.num_models):
                ...
                grad_outputs = torch.ones(predicted.shape, device=device)
            ...
            grad_l2_norm_ = grad_l2(models, device)
            if is_rank0 and (np.isnan(grad_l2_norm_) or np.isinf(grad_l2_norm_)):
                print(f"NaN / Inf gradient values!!")
            grad_l2_norm.update(grad_l2_norm_)

            if args.amp and args.ddp:
        ...
        if i % args.print_freq == 0 and is_rank0:
            ...
            print(f"Grad Scaler: {scaler.get_scale()}")
