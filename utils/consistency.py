...
def consistency_loss(logits: List[torch.Tensor], lbd: float, eta: float = 0.5, loss='default'):
    """
    ...
    loss : {'default', 'xent', 'kl', 'mse'} (optional)
        ...
        - 'kl': The KL-divergence between each prediction and their average.
...
def kl_div(input_, targets):
    return F.kl_div(F.log_softmax(input_, dim=1), targets, reduction='none').sum(1)


def entropy(input_):
    logsoftmax = torch.log(input_.clamp(min=1e-20))
    xent = (-input_ * logsoftmax).sum(1)
    return xent
